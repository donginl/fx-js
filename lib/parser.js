            ...
            switch(brand.toUpperCase()) {
                case 'V':
                    ...
                    waterfall([
                        ...
                        (response, body, done) => {
                            if (response.statusCode != 200) return done(Error('server is currently unavailable'));

                            let rate = JSON.parse(body).Exchange.exchRate;
                            if (!rate) return done(Error('exchange rate was not announced'));
                ...
                case 'MA':
                    ...
                    waterfall([
                        ...
                        (response, body, callback) => {
                            if (response.statusCode != 200) return callback(Error('server is currently unavailable'));
